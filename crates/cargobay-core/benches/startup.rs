//! Micro-benchmarks for cargobay-core hot paths.
//!
//! Run with:
//!   cargo bench -p cargobay-core
//!
//! Results are written to `target/criterion/` with HTML reports.

use criterion::{black_box, criterion_group, criterion_main, Criterion};

use cargobay_core::hypervisor::{VmInfo, VmState};
use cargobay_core::store::VmStore;
...
fn bench_create_hypervisor(c: &mut Criterion) {
    c.bench_function("create_hypervisor", |b| {
        b.iter(|| {
            let _hv = black_box(cargobay_core::create_hypervisor());
        });
    });
}
...
fn bench_platform_info(c: &mut Criterion) {
    c.bench_function("platform_info", |b| {
        b.iter(|| {
            let _info = black_box(cargobay_core::platform_info());
        });
    });
}
