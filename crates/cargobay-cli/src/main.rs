...
async fn main() {
    ...
    match cli.command {
        ...
        Commands::Completions { shell } => {
            ...
            clap_complete::generate(shell, &mut cmd, "cargobay", &mut std::io::stdout());
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn parse_docker_ps() {
        let cli = Cli::try_parse_from(["cargobay", "docker", "ps"]);
    ...
    #[test]
    fn parse_vm_list() {
        let cli = Cli::try_parse_from(["cargobay", "vm", "list"]);
    ...
    fn parse_vm_create_with_defaults() {
        let cli = Cli::try_parse_from(["cargobay", "vm", "create", "my-vm"]).unwrap();
        match cli.command {
            Commands::Vm {
                command:
                    VmCommands::Create {
    ...
    fn parse_image_search() {
        let cli = Cli::try_parse_from(["cargobay", "image", "search", "nginx"]).unwrap();
        match cli.command {
            Commands::Image {
                command:
                    ImageCommands::Search {
    ...
    #[test]
    fn parse_k3s_status() {
        let cli = Cli::try_parse_from(["cargobay", "k3s", "status"]);
    ...
    fn parse_volume_list() {
        let cli = Cli::try_parse_from(["cargobay", "volume", "list"]);
    ...
    fn parse_volume_create() {
        let cli = Cli::try_parse_from(["cargobay", "volume", "create", "myvolume"]).unwrap();
        match cli.command {
            Commands::Volume {
    ...
    #[test]
    fn parse_status() {
        let cli = Cli::try_parse_from(["cargobay", "status"]);
