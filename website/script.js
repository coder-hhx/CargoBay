(() => {
  ...
  const translations = {
    en: {
      ...
      summary3Body:
        "Installers ship ready-to-run runtime assets on macOS (lightweight VM), Linux (bundled QEMU/KVM guest), and Windows (WSL2) — no Docker Desktop, Colima, docker-cli, compose, or system QEMU required, and the Windows runtime now ships an Alpine WSL distro with OpenRC-managed Docker services, waits briefly for the direct guest endpoint before falling back to localhost or a local relay, falls back once to a detached compatibility-safe wsl.exe dockerd bootstrap if the API still does not come up, prefers the host-reachable WSL IP over bridge-only addresses, validates readiness through the Docker API itself, and can emit optional progress logs for WSL diagnostics.",
