(() => {
  ...
  const translations = {
    en: {
      ...
      statusNote:
        "The preview now ships with automated UI, Docker runtime, AI runtime, Linux desktop runtime validation, local GPU telemetry for model and sandbox runtimes, safer confirmation gates for destructive sandbox actions, and clearer categorized sandbox error messaging; the desktop dashboard and navigation now prioritize sandboxes, models, MCP, settings-driven bridges, and controlled provider canaries, while VMs and Kubernetes remain post-v1 tracks.",
    ...
    zh: {
      ...
      statusNote:
        "当前预览版本已补齐自动化 UI、Docker runtime、AI runtime、Linux 桌面运行时验证、面向本地模型与沙箱运行时的 GPU 遥测，并为高风险沙箱操作加入更安全的确认门槛和分类更清晰的错误提示；桌面 dashboard 与导航现已优先聚焦沙箱、模型、MCP、由设置驱动的 bridges，以及受控 provider canary，VM 与 Kubernetes 则放到 v1 之后。",
      footer: "CrateBay · <span data-year></span>",
    },
  }
